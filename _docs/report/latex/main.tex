% =========================================================================================
% POCUS-Emergency -- End-of-Year Internship Report (PFA)
...
\begin{titlepage}
  \begin{center}
    ...
    % One logo, because there is only one institution on this page. The host organisation is
    % Esprit Tech, the research entity OF ESPRIT -- not a separate company -- so a second
    % logo slot beside this one had nothing to put in it. It used to sit here, permanently
    % empty, and its absence was repeatedly mistaken for a missing asset.
    %
    ...
    \vspace{1.1cm}
    {\Large\bfseries End-of-Year Internship Report (PFA)\par}
...
I am grateful to ESPRIT and to Esprit Tech for the opportunity to carry out an end-of-year
...
% "Context", not "Mise en contexte". The report is written in English, and this was the one
% heading left in French. The grouping the supervisor asked for is unchanged -- introduction
% and project environment, before the methodology begins -- only its name is translated.
\part{Context}
...
\include{chapters/04b-data-preparation}
\include{chapters/05-perception-agents}
\include{chapters/06-clinical-reasoning}
% Evaluation before Deployment, which is CRISP-DM's order. The files keep their original
% numbering: renaming them would rewrite every path in the repository's history for a gain
% that is invisible in the compiled document, where the chapter numbers come from LaTeX.
