...
The work was carried out at \textbf{Esprit Tech}, the research entity attached to ESPRIT
(\textit{École Supérieure Privée d'Ingénierie et de Technologies}). Esprit Tech proposes a
set of applied research subjects each year and supervises them as end-of-studies internships,
which places a project of this kind in a research setting rather than a product one: the
objective is a defensible investigation of whether an approach works, and the deliverable is
the evidence for that answer as much as the software itself.

That framing shaped several decisions reported later. It is the reason a negative result is
treated as a result --- seven well-motivated changes were tested and rejected on measurement
(Section~\ref{subsec:results-rejected}) --- and the reason the system is built to state what
it does not know rather than to appear complete.

The clinical framing of the subject was set in partnership with the \textbf{STMU}
(\textit{Société Tunisienne de Médecine d'Urgence}), the Tunisian society for emergency
medicine. The partnership defines the clinical problem the work addresses: point-of-care
ultrasound in the emergency department, where the constraint is not imaging quality but the
speed and completeness of the decision made around it. It is the source of the framing that
runs through this report --- that an unperformed examination and a negative one are different
facts, and that a decision-support system which conflates them is worse than none.

The clinical scope of this report should be read with one limitation stated plainly here
rather than left to the conclusion: no part of the system was validated against patient
outcomes, and no clinician used it in a live encounter during the internship. The partnership
established what the problem is and what a safe answer would have to look like; it did not
provide a prospective evaluation, which remains the principal item of future work
(Chapter~\ref{sec:conclusion}).

\subsection{Internship Environment}
\label{subsec:internship-env}

The project was carried out as an individual end-of-studies internship from July to
mid-September 2026, with the scope of a minimum viable product (\gls{mvp}) rather than a
...
execution reserved for data preparation, analysis and inference.

Supervision took the form of a fixed weekly meeting each Monday, held on site or online as
circumstances required, at which the previous week's work was reviewed and the next week's
priorities agreed. A weekly cadence suited the constraint described below: with GPU time
intermittent, a training run committed on a Monday had to be worth the week it occupied, and
the meeting was where that judgement was made. Several of the rejections reported in
Chapter~\ref{sec:results} were decided at one --- the value of a fixed review point being
that a negative result is discussed rather than quietly retried.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{lXX}
    ...
    Triage Agent & Chief complaint, vital signs, age, red-flag symptoms &
      Urgency level with calibrated confidence. Built and evaluated as described in
      Section~\ref{subsec:triage-agent}; in the deployed application the tier is supplied by
      the clinician instead (Section~\ref{subsec:triage-deployment}) \\
    \addlinespace[2pt]
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{lXX}
    ...
    \addlinespace[2pt]
    Deployment & The system served as a running application: a web service, a clinician-facing
      interface, containerisation, dependency management, automated testing, security scanning
      and request monitoring (Chapter~\ref{sec:integration}) &
      Carried out as \emph{technical} deployment. The intended users are emergency physicians
      and the system is built for that use; what was not undertaken is installation in a
      hospital and prospective clinical validation \\
    \bottomrule
  \end{tabularx}
...
\begin{figure}[H]
  ...
  \caption{The CRISP-DM cycle as followed here. The return path from modelling and
    ...
    route through the cycle rather than an exception. Deployment was reached: the system runs
    as a served application built for use by clinicians. Prospective validation in a hospital
    lies beyond the cycle shown.}
  \label{fig:crispdm}
\end{figure}
