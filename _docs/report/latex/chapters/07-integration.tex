...
\begin{enumerate}
  \item \textbf{Each agent is independently testable.} The reasoning agent can be exercised
        ...
        209 tests of Section~\ref{subsec:reasoning-safety} run --- in seconds, with no GPU and
        no model weights.
...
The interface is built around that asymmetry. Rather than replaying fixed cases, it lets the
user construct a patient --- triage tier, which organs were requested, which were actually
scanned, each finding and its confidence, and for every vital and laboratory value a checkbox
that makes it \emph{never measured} rather than normal. The clinical state, the conflicts, the
list of what is absent, the evidence identifiers and the escalation decision are recomputed on
every change, with no model and no GPU involved.

Three things about it are worth reporting.
...
\paragraph{Live and recorded are labelled distinctly.} The differential requires a 4.9\,GB model
on a GPU, so for the five benchmark cases the interface displays the answers recorded from the
frozen run of Section~\ref{subsec:reasoning-ab}, marked \textsc{recorded} with their measured
latency, while everything computed in the browser is marked \textsc{live}. An interface that
blurred the two would misrepresent what the machine in front of the examiner is doing.
