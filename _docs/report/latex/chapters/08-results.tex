...
does. This is demonstrated rather than asserted: 228 tests exercise the safety layer with no
...
\paragraph{Two of the three per-organ inference functions are still not under test.} This
limitation was previously total: every test built module reports directly, none imported the
per-organ prediction code, and the suite therefore proved the contract was \emph{enforced}
without proving that any module \emph{honoured} it.

It has been closed for the lung module, whose prediction code was extracted from the notebook
and is now exercised by thirteen tests running real inference on the shipped checkpoints
(Section~\ref{subsubsec:agent-extraction}). Closing it immediately exposed a defect that had
made the module silently report every study as negative, which is the strongest available
argument that the limitation was worth closing rather than merely declaring.

Cardiac and gallbladder inference remains outside the suite, because their weights are not held
in the repository. For those two modules the original statement stands: the contract is proven
