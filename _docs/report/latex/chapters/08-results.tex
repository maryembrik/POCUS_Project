...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}lXlll@{}}
    ...
    \midrule
    Triage & 3-tier urgency, tabular & macro F1 0.666 & 0.333 (chance) & 16\,180 enc. \\
    Lung & 4 findings, multi-label & macro AUROC 0.788 & 0.500 (chance) & 165 \\
    Cardiac & segmentation $\rightarrow$ EF & Dice 0.93 / 0.83 / 0.89 & --- & 100 patients \\
    Gallbladder & 5 pathology classes & bal.\ acc.\ 63.1\% & 20.0\% (chance) & 199 \\
    \bottomrule
  \end{tabularx}
\end{table}

Read that way, the modules are more alike than their headline figures suggest. The gallbladder
classifier reaches roughly three times its chance level on a five-class problem; the triage
model twice its chance level on a three-class one; the lung classifier sits well above the
uninformative 0.5 on every finding it models. None of them is close to the accuracy a
deployed clinical tool would require, and all of them are clearly doing something other than
guessing.

The cardiac module has no chance-level baseline because segmentation is not a classification
task. Its derived ejection fraction carries a mean absolute error of 7.1 percentage points,
which is the figure a clinician would actually act on, and
Section~\ref{subsec:cardiac-results} reports what that error means against the bands used to
call function normal, reduced or severe.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}lXl@{}}
    ...
    \midrule
    External-dataset integration & \rmq{one line: what was added, what it cost} & rejected \\
    ...
    Triage class weighting & Aggressive weighting would improve safety on the
      ...
      rejected \\
    Reason-for-visit grouping & A coarser grouping would generalise across sources;
      \rmq{measured outcome} & rejected \\
    \bottomrule
  \end{tabularx}
\end{table}
...
does. This is demonstrated rather than asserted: 180 tests exercise the safety layer with no
...
\paragraph{The per-organ inference functions are not under test.} The 180 tests build module
reports directly and none of them imports the per-organ prediction code, which lives in a
notebook. The suite proves the contract is enforced, not that the ultrasound agent honours it;
that link is currently verified by running the notebook rather than by the test suite.
...
classifier at 63.1\% balanced accuracy on five classes is the clearest example: as a
