...
Within the second part the chapters follow the order in which the system was built, so that
each depends only on what precedes it. That ordering places integration before the synthesis
of results, which inverts the usual sequence of the two phases; it is deliberate. The
evaluation of each module is reported inside the chapter that builds it, where the reader has
the method in view, and Chapter~\ref{sec:results} draws those results together across modules
once the assembled system exists to be discussed.
...
\begin{itemize}[label=$\bullet$]
  ...
  \item \textbf{Chapter~\ref{sec:perception} --- Modeling: Perception Agents.} Development
    and evaluation of the triage, lung, cardiac and gallbladder modules, including the
    experiments that failed and the corrections they prompted.
  \item \textbf{Chapter~\ref{sec:reasoning} --- Modeling: Clinical Reasoning Agent.} The
    clinical state representation, conflict detection, retrieval, the medical language model,
    output validation and the escalation policy.
  \item \textbf{Chapter~\ref{sec:integration} --- Deployment: System Integration and
    Prototype.} The end-to-end pipeline, the contract through which the agents communicate,
    worked clinical scenarios, and how the assembled system is served.
