...
% =========================================================================================
\section{Modeling: Perception Agents}
...
\subsubsection{What is Deployed, and What is Not}
\label{subsec:triage-deployment}

\begin{keystatement}
This classifier was built, calibrated and evaluated. It is not called by the deployed
application. In the running system the urgency tier is entered by the clinician.
\end{keystatement}

The reason is a mismatch between what the model needs and what the clinical intake collects.
The features that carry most of its signal --- arrival mode, and the coded reason-for-visit
categories --- are administrative fields recorded by a hospital registration system. The intake
screen of this application asks a clinician for a presenting complaint in free text. Connecting
the two would mean inferring a coded category from a free-text complaint, and a tier predicted
from features that were guessed at is not the tier the reported accuracy describes.

Presenting a number produced that way as ``calibrated'' would be the more serious error, since
the whole purpose of calibrating this output was to make it comparable with an imaging
confidence. So the deployed interface offers the clinician a three-way control ---
\textsc{low}, \textsc{medium}, \textsc{high} --- and records the tier with its provenance, as
\texttt{model: "clinician"}, so that a reader of an archived encounter can tell a stated
assessment from a predicted one.

\paragraph{Why this is not merely a gap.} The tier is not decoration in the reasoning layer.
One of the escalation triggers compares it against the imaging findings: when triage says
\textsc{low} and a module reports something severe, the two disagree, and the system escalates
rather than choosing between them. A clinician-entered tier is a sound input to that
comparison --- arguably a better one than a prediction, since it is a second independent
judgement rather than another model reading the same record.

\paragraph{A defect this exposed.} The control described here did not exist until late in the
project. The intake form carried a fixed value of \textsc{medium} and sent it with every
encounter, so the disagreement trigger could not fire for any patient entered through the
interface. The rule itself was correct and its own tests passed throughout, because those
tests call the conflict detector directly with a \textsc{low} tier. A rule can be correct and
still never run, and no test that exercises the rule can reveal that: it took following the
value from the screen to the reasoning layer. The end-to-end checks added with the control
now traverse that path over HTTP, which is where the gap was.
