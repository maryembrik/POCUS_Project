...
It has since been closed for the lung module, whose checkpoints are held in the repository. The
prediction code was extracted into an importable module and is now exercised by thirteen tests
grouped under a new safety property, \emph{perception contract}.
...
The cardiac and gallbladder modules remain outside this treatment: their weights are not held in
the repository, so the agent returns \texttt{not\_supported} for both rather than a guess. Their
class names and clinical groups, however, now live in the same importable module as the lung
findings, so the output contract has a single home rather than being restated in a notebook.
