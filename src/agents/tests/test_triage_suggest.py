...
def _needs_model() -> None:
    ...
    `pytest.mark.skipif` is invisible to run_benchmark.py, which calls each test as a plain
    callable -- so a marker alone left these seven failing there in any checkout without the
    artefact, while pytest reported them cleanly skipped. `unittest.SkipTest` is honoured by
    pytest and is caught by the benchmark, which records the test as skipped rather than as
    passed. A test that could not run has not passed, and the benchmark table says so.

    The artefact is absent by design: it is 6 MB, the repository does not carry weights, and
    `python -m src.agents.triage.deployed` rebuilds it from the committed manifest.
    """
    if not sg.available():
        raise unittest.SkipTest(
            "the deployed triage model is not in this checkout; rebuild it with "
