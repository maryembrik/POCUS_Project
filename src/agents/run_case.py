"""Run one encounter end to end through the Clinical Reasoning Agent, with a real model.
    ...
    python -m src.agents.run_case              # the missing-data scenario
    python -m src.agents.run_case --scenario conflict
    python -m src.agents.run_case --dry-run    # no model; prompt and escalation only
...
def main() -> int:
    ...
    t0 = time.time()
    out = reason(state, llm_fn=llm_fn, max_revisions=args.max_revisions)
